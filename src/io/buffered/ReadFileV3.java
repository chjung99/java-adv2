...
public class ReadFileV3 {
    public static void main(String[] args) throws IOException {
        ...
        while ((data = bis.read()) != -1) {
            // read는 1byte만 조회한다
            // BufferedInputStream은 먼저 버퍼를 화인한다. 버퍼에 데이터가 없으므로 데이터를 불러온다
            // BufferedInputStream은 FileInputStream에서 read(byte[])을 사용해서 버퍼의 크기인 3byte 데이터를 불러온다
            // 불러온 데이터를 버퍼에 보관한다.
            // read()요청이 오면 버퍼에서 바로 1byte씩 준다
            // 예제 3이 예제2보다 느린 이유는 동기화 코드 때문
            // BufferedXxx 클래스는 멀티 스레드를 고려해서 만든 클래스로 동기화 코드로 인해 성능이 약간 저하될 수 있다
            fileSize ++;
        }
        bis.close();
